...
class Config:
    # -------- Paths --------
    raw_root: Path = Path("/gemini/space/users/tong/data/cube_sponge")
    output_root: Path = Path("/gemini/space/users/tong/data/cube_sponge/lerobot_100")

    # -------- Dataset --------
    repo_id: str = "Loki0929/teleai_umi_100"
    task: str = "Grasp the cube and sponge into the bin"
    fps: int = 30

    # -------- Limit --------
    max_sessions: Optional[int] = 100   # ← 改这里：100 / 200 / None(全部)
...
def main():
    ...
    sessions = sorted(
        p for p in CFG.raw_root.iterdir()
    ...
    print(f"[INFO] Sessions used: {len(sessions)} / total={len(list(CFG.raw_root.iterdir()))}")
