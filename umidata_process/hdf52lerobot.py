...
def _load_images(images_group: h5py.Group, cameras: List[str]) -> Dict[str, np.ndarray]:
    ...
    for cam in cameras:
        ...
        if ds.ndim == 4:
            arr = ds[:]             # (T,H,W,3)
            # arr = arr[..., ::-1]    # BGR -> RGB hdf5 已经是 RGB了
        else:
...
def convert_pick_lid_data():
    port_fastumi(
        raw_dir=Path('/gemini-1/space/tong/data/cube_sponge/hdf5'),
        repo_id="Loki0929/teleai_umi",
        task="Grasp the cube and sponge into the bin",
        fps=30,
        mode="video",
        push_to_hub=False,
        workers=40,
        output_dir=Path('/gemini-1/space/tong/data/cube_sponge/lerobot'),
    )
